...
from app import db
from app.catalog.model import Book, Publication
from flask import render_template, flash, request, redirect, url_for
from flask_login import login_required
from app.catalog.froms import EditBookForm, CreateBookForm
...
@main.route('/book/delete/<bookid>',methods=['GET','POST'])
@login_required
def delete_book(bookid):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        flask('book delete suceessfully')
        return redirect(url_for('main.display_book'))
    return render_template('delete_book.html',book=book,bookid=book.id)

@main.route('/edit/book/<bookid>', methods=['GET','POST'])
@login_required
def edit_book(bookid):
    ...
    if form.validate_on_submit():
        ...
        book.num_page = form.page_num.data
        db.session.add(book)
        db.session.commit()
        flask('Book Edited Successfully')
        return redirect(url_for('main.display_book'))
...
def create_book(pub_id):
    ...
    if form.validate_on_submit():
        book = Book(
            ...
            avr_rating=form.avg_rating.data,
